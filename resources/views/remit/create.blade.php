...
@section('content')
    ...
    <form action="{{ route('remit.store') }}" method="POST" novalidate>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('remit_date') is-invalid @enderror"
                   id="remit_date" name="remit_date" value="{{ old('remit_date') }}">
            @error('remit_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
